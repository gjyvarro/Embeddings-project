...
We also make an important observation: larger dimensionality usually leads to less negative optimal curvature.
We propose a new estimator of global graph curvature which, in contrast to the existing ones, reflects this property as it is dimension-specific.
We demonstrate that for a given dimension the new estimator is able to distinguish between ``hyperbolic'' and ``Euclidean'' networks. 
...
We approach this problem by introducing a new curvature estimator which is, in contrast to all existing ones, is dimension-specific. This estimator agrees with our theoretical results and is also able to distinguish between ``hyperbolic'' and ``Euclidean'' networks.
...
A nice feature of the proposed estimator is that it depends on dimension. Moreover, this dependence is natural: when $d$ becomes larger, curvature becomes smaller. As we show in the experiments on various datasets, similar property is observed for the optimal curvature.
...
We choose distortion for this illustrative example, since for this loss we theoretically obtained non-trivial result: for some simple graphs the optimal curvature is negative, while for other it is positive.
...
Indeed, we observe such expected behavior for two illustrative examples shown on Figures~\ref{fig:star} and~\ref{fig:stars}. On Figure~\ref{fig:star} we take a cycle of length 10 and iteratively add edges to one of its nodes, i.e., we create a star on one node. On Figure~\ref{fig:stars} we take the same cycle and add small stars sequentially to several its nodes: on the left figure we just add one dangling edge for each node (i.e., create a star subgraph on 4 nodes), on the right we add two edges. In all cases, the optimal curvature drifts from a positive one for a cycle to negative values. This drift is more pronounced on Figure~\ref{fig:star}, where we create one but large star.
